...
# By the time you make it to sub-basement 17, there are only a few minutes until 
# midnight. "We have a big problem," she says; "there must be almost fifty bugs 
# in this system, but nothing else can print The List. Stand in this square, 
...
def solve_captcha(input, n=1):
  input = str(input)
  sum = 0
  newinput = input + input[0:n]
  for i in range(0, len(input)):    
    if(input[i] == newinput[i+n]):
      sum += int(input[i])
  return sum

if __name__ == '__main__':
    if (len(argv) == 2):
      print(solve_captcha(argv[1]))
      print(solve_captcha(argv[1], 2))
    else:
      file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "inputs/day01.txt")
      f = open(file_path)
      data = f.read()
      print(solve_captcha(data))
      print(solve_captcha(data, 2))
